...
const css = (theme) => {
  return theme === 'dark' ? 'background-color:black; color:white;' : 'background-color:white; color:black;';
};
...
exports.home = (req, res) => {
  ...
  const buttons = `<a href='topic/create'>create</a>`;
  const authStatusUI = req.session.is_logined
  ...
  const html = template.HTML(
    ...
    list,
    buttons,
    `<h2>${title}</h2>${description}<img src='./images/hello.jpg' style="width:500px; display:block;margin-top:10px">`,
    css(req.cookies.theme),
    authStatusUI
  );
...
exports.create = (req, res) => {
  db.query(`SELECT id, name FROM author`, (err1, authors) => {
    ...
    `;
    const html = template.HTML(title, list, buttons, form, css(req.cookies.theme));
